...
>
        ...
        {{ props.row.sum_quantity }}
                            ...
                            <template #actions="props">
                                <div class="d-flex justify-content-center align-items-center">
                                    <!-- View icon -->
                                    <a href="javascript:;" class="view me-3" data-bs-toggle="modal" data-bs-target="#viewModal" @click="viewSpareItems(props.row.id),spare = props.row "> <!-- Changed props.row to props.row.id -->
                                        <svg
  ...
  <div class="modal-dialog modal-xl"> <!-- Changed from modal-dialog to modal-dialog modal-xl -->
    <div class="modal-content">
      <div class="modal-header d-flex justify-content-between align-items-start">
        <div class="d-flex flex-column gap-1">
          <h1 class="modal-title fs-5 mb-2" id="viewModalLabel">View Spare Details</h1>
        </div>
      ...
      <div class="modal-body">
       <div class="row layout-top-spacing">
            <div class="col-xl-12 col-lg-12 col-sm-12 layout-spacing">
                <div class="card mb-3">
                    <div class="card-body">
                        <h5 class="card-title text-primary mb-3">Spare Details</h5>
                        <div class="d-flex justify-content-between align-items-center">
                            <p class="h6 mb-0" style="font-family: 'Roboto', sans-serif; font-size: 1.1rem;"><strong>Spare Name:</strong> <span class="text-dark">{{ spare.name }}</span></p>
                            <p class="h6 mb-0" style="font-family: 'Open Sans', sans-serif; font-size: 1.1rem;"><strong>Total Quantity:</strong> <span class="badge bg-success" style="font-size: 1.2rem; padding: 8px 12px;">{{ spare.sum_quantity }}</span></p>
                        </div>
                    </div>
                </div>
                <div class="panel p-0">
                    ...
                    <div class="custom-table table3">
                        <div v-if="modalLoading" v-html="loadingIndicator()"></div>
                                   <v-client-table :data="SpareItems" :columns="SpareItemsColumns" :options="table_option" >
                                   <template #SNo="{ index }">
    {{ index + 0 }}
</template>
                                ...
                                <div class="d-flex justify-content-center align-items-center">
                                    ...
                                    <a href="javascript:void(0);" title="Delete" data-bs-toggle="tooltip" data-bs-placement="top">
                                        <svg
                                            ...
                                            stroke-linejoin="round"
                                            class="text-danger"
                                            @click="deleteSpareItem(props.row)"
                                        >
                                            <polyline points="3 6 5 6 21 6"></polyline>
                                            <path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path>
                                        </svg>
    ...
    <div class="modal-content">
      ...
      <form @submit.prevent="createLpo()">
        ...
        <div class="row mt-2">
          ...
          <div class="col-md-11">
            <input type="text" class="form-control mt-1" placeholder="Enter description" v-model="description">
          </div>
         ...
         <!-- approval chain ends -->
          <!-- items list starts -->
            ...
            <div class="col-xl-12 col-lg-12 col-sm-12 layout-spacing">
                <div class="panel p-0">
                    <div class="custom-table table3">
                                   <v-client-table :data="previewItems" :columns="lpoColumns"  >
...
<script setup>
    ...
    const quantity = ref('')
    const Spares = ref([]);
    const SpareItems = ref([]);
const SpareItemsColumns = ref([ 'SNo', 'Quantity', 'LPO', 'Requested By', 'Order Date', 'Vendor', 'Location', 'Phone']);
    ...
    const editForm = ref({id: null, name: '', part_number: '' , unit :''}); 
    const loading_spinner = ref(false);
    const columns = ref(['SNo', 'name','part_number','unit','actions']);
    const lpoColumns = ref(['index','name','unit','quantity','description','rate','total','actions'])
...
const viewSpareItems = (spare_id) => {
    SpareItems.value = [];
    modalLoading.value = true;
    axiosInstance.get(`/spares/${spare_id}`).then((response) => {
        const spare = response.data.data;
        if (spare && Array.isArray(spare.store_items)) {
            SpareItems.value = spare.store_items.map((item, idx) => {
                const po = item.item?.purchase_order;
...
return {
    ...
    'Order Date': formatShortDate(po?.date_created),
    'Vendor': vendor?.name ?? '',
