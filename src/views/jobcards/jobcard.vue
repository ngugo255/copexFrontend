...
<td v-if="jobcard">
  {{ machine?.name && machine?.type_name ? machine.name + ' - ' + machine.type_name : machine?.name || '' }}
</td>
...
/>
    ...
    <input
        ...
        class="form-control"
        :placeholder="spare_usage ? 'Available ' + (spare_usage.balance_quantity ?? '') : 'Select Spare'"
        min="1"
        :max="spare_usage?.balance_quantity ?? undefined"
        required
    />
    </div>
    ...
    const registerItemUsage = async () => {
        ...
        try {
            const usageData = {
                ...
                item_id:spare_usage.value.item_id,
...
const getSpares = async () => {
    try {
        ...
        if (Array.isArray(response.data)) {
          spares.value = response.data.map((item, index) => ({
    ...item,
        ...
        <body>
            ...
            <div class="section">
                ...
                <table class="horizontal-table">
                    ...
                    <tr>
                     ...
                     <td>${machine.value?.name && machine.value?.type_name ? machine.value.name + ' - ' + machine.value.type_name : machine.value?.name || ''}</td>
            ...
            <div class="section">
                ...
                ${items.value.length > 0 ? 
                    '<table><thead><tr><th>S.No</th><th>Spare</th><th>Description</th><th>Unit</th><th>Quantity</th></tr></thead><tbody>' +
                    items.value.map(item => 
                        '<tr><td>' + item.SNo +  '</td><td>' + item.spare + '</td><td>' + item.description + '</td><td>' + item.unit + '</td><td>'
                         + item.quantity + '</td></tr>'
                    ).join('') +
                    '</tbody></table>'
                : '<p>No spares used</p>'}
