<template>
  ...
  <path fill-rule="evenodd" d="M15 8a.5.5 0 0 0-.5-.5H2.707l3.147-3.146a.5.5 0 1 0-.708-.708l-4 4a.5.5 0 0 0 0 .708l4 4a.5.5 0 0 0 .708-.708L2.707 8.5H14.5A.5.5 0 0 0 15 8"/>
                ...
                <button v-if="user_role != 2 " class="btn btn-primary me-2" @click="filling = !filling">
                        ...
                        <table class="table">
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td>{{ details.quantity }} Litres</td>
                                    <td>{{ (details.rate *1).toLocaleString() }}</td>
                                    <td>{{ (details.rate * details.quantity).toLocaleString() }}</td>
...
<script setup>
    ...
    //table 3
    const columns = ref(['SNo', 'date', 'fuel_used','hrs_Km_difference','supervisor', 'task','initial_reading','final_reading','ratio','actions']);
    const items = ref([]);
    const details= ref('');
    const loading = ref('');
    ...
    const getBulkDistribution = async () => {
        loading.value=true;
        try {
            const response = await axiosInstance.get(`/items/${route.params.distribution_id}`);
            if (response.data ) {
                details.value=response.data;
                loading.value=false;
                console.log("statement details ",details.value);
            } else {
    ...
    items.value = response.data.map((distribution, index) => ({
        ...
        hrs_Km_difference: distribution.final_reading == 0.00 ? "---" : (distribution.final_reading - distribution.initial_reading).toFixed(2),
        ...
        ratio: distribution.final_reading == 0.00 ? "---" : 
