                ...
                <div class="panel p-0">
                    <div class="custom-table table3">
                        <v-client-table :data="labour_types" :columns="columns" :options="table_option" >
                            <template #labour_status="props">
                                <span
                                    :class="props.row.labour_status === 'Unapproved' ? 'p-1 bg-danger' : 'p-1 bg-success'"
                                >
                                    {{ props.row.labour_status }}
                                </span>
                            </template>
...
<a v-if="user_permissions.includes('can_edit_wages')" href="javascript:void(0)" class="text-primary me-2" title="Edit" @click="openEditModal(props.row)" data-bs-toggle="modal" :data-bs-target="'#modal-' + props.row.id">
  ...
  <a v-if=" props.row.labour_status === 'Unapproved' &&  user_permissions.includes('can_approve_wages') "  href="javascript:void(0)" class="text-success" title="Approve" 
  ...
  <!-- approval eye  starts -->
     ...
     @click="approvalLabourType(props.row.id)"  
    >
<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="text-primary bi bi-eye" viewBox="0 0 16 16">
  ...
  <path d="M8 5.5a2.5 2.5 0 1 0 0 5 2.5 2.5 0 0 0 0-5M4.5 8a3.5 3.5 0 1 1 7 0 3.5 3.5 0 0 1-7 0"/>
   ...
   <!-- approval eye ends -->
 <a  v-if="props.row.labour_status !== 'Unapproved' && user_permissions.includes('can_approve_wages')" href="javascript:void(0)" class="text-warning ms-2" title="Unapprove" @click="unapprovalLabourType(props.row.id)">
...
<script setup>
    ...
    const columns = ref(['Sn','labour_type_name','rate','labour_status','actions']);
        ...
        loading_spinner.value = true;
         ...
         then((response) => {          
               labour_types.value = response.data.map((labour_type, index) => {
                ...
                labour_status:labour_type.status , 
