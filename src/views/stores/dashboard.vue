...
<!-- store item edit modal starts -->
<div class="modal fade mymodal" id="edit_role" data-bs-backdrop="false" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <h1 class="modal-title fs-5" id="staticBackdropLabel">STORE ITEM EDITING</h1>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close" id="close" @click="clearItemForm"></button>
      </div>
      <form @submit.prevent="updateStoreItem()">
      <div class="modal-body">
        <div class="row mb-2">
            <div class="col-md-6">
                <label class="form-label">Item Name</label>
                <input type="text" class="form-control" v-model="editItemName" placeholder="Enter Item Name" required>
            </div>
            <div class="col-md-6">
                <label class="form-label">Quantity</label>
                <input type="text" class="form-control" v-model="editItemQuantity" placeholder="Enter Quantity" required>
            </div>
                            ...
                            <template #actions="props">
                               ...
                               href="javascript:void(0);" title="Delete" data-bs-toggle="tooltip" data-bs-placement="top">
                                   <svg
                                       ...
                                       stroke-linejoin="round"
                                       class="text-danger"
                                       @click="deleteStoreItem(props.row.id)"
                                   >
...
<script setup>
    ...
    const itemsColumns = ref(['SNo', 'name','used','balance','total','actions']);
    ...
    const updateStoreItem = () => {
        ...
        axiosInstance.put(`/store-items/${editItemId.value}`, itemData)
            .then((response) => {
                getStoreItems(store.value.id);
                sending.value = false;
                document.getElementById('close').click();
                document.getElementById('back-modal').click();         
                showMessage('Store item updated successfully!');
            })
            .catch((error) => {
                sending.value = false;
                showError(error.message);
    ...
    const deleteStoreItem = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                loading_spinner.value = true;
                axiosInstance.delete(`/store-items/${id}`)
                    .then((response) => {
                        getStoreItems(store.value.id);
